...
<script>
	...
	export default {
		...
		onLoad(options) {
			...
			S_detailBills = uni.getStorageSync("detailBills")
			S_id = options.id
			...
			for (let i = 0; i < this.products.length; i++) {
				...
				for (let good of S_detailBills) {
					if (good.goodsId.objectId == this.products[i].objectId) {
						good.enterNum = this.products[i].num
					}
				}
			}
									...
									query.save().then(res => {
										common.enterAddGoodNum(that.products).then(result => { //添加产品数量
											setTimeout(() => {
												...
												common.log(uni.getStorageSync("user").nickName + "生产入库了'" + that.products[0].goodsName + "'等" +
													that.products
