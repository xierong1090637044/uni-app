		...
		<view class="content">
			<view class="left_content">
				<view style="height: calc(100vh - 80rpx);overflow: scroll;">
					<view v-for="(item,index) in customFristClass" :key="index" :class="selected_id===item.objectId?'selectd_item':''"
			...
			<view class="right_content">
				<view style="height: calc(100vh - 80rpx);overflow: scroll;">
					<view v-for="(item,index) in customSecondClass" :key="index" class="class_item" style="color: #333333;">
						<view class="class_texxt_view">{{item.class_text}}</view>
	...
	export default {
		...
		data() {
			return {
				is_choose:false,
				loading: true,
				customFristClass: null, //一级分类
				customSecondClass: null, //二级分类
		...
		onLoad(options) {
			that = this;
			uni.setNavigationBarTitle({"title":"客户类别"})
		...
		methods: {
			...
			select_this(item,type){
				item.type = type
				uni.setStorageSync("customCategory", item)
				uni.navigateBack({
					delta: 1
				})
			},
			...
			get_category(){
				...
				const query = Bmob.Query("customFristClass");
				query.equalTo("parent", "==", uid);
				query.find().then(res => {
					console.log(res)
					that.customFristClass = res;
			...
			get_second_category(id) {
				...
				query.relation('customSecondClass').then(res => {
					//console.log(res);
					uni.hideLoading()
					that.customSecondClass = res.results;
				})
			},
			...
			showoption(id, type, value) {
				...
				uni.showActionSheet({
					...
					success: function(res) {
						...
						if (res.tapIndex == 0) {
							if (type === 1) {
								that.delete(id, "customFristClass")
							} else {
								that.delete(id, "customSecondClass")
							}
			...
			edit() {
				...
				if (that.Popup_title === "一级分类") {
					dbname = "customFristClass";
				} else {
					dbname = "customSecondClass";
				}
			...
			confrim() {
				...
				if (this.Popup_title === "一级分类") {
					...
					const pointer = Bmob.Pointer('customFristClass')
					const poiID = pointer.set(selected_id)

					const query = Bmob.Query('customSecondClass');
					query.set("class_text", class_text)
					query.set("parent", poiID)
					query.save().then(res => {
						//console.log(res)
						const relation = Bmob.Relation('customFristClass') // 需要关联的表
