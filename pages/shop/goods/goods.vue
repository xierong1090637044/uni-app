<template>
	<view>
		...
		<view class="footer">
			<view class="icons">
				<view class="box" @tap="share">
					<fa-icon type="share" size="20" color="#999" />
					...
					<view class="length">
						...
						<view class="number">
							<view class="sub" @tap.stop="sub">
								<fa-icon type="minus" size="20" color="#999" style="line-height: 60upx;" />
							</view>
							<view class="input" @tap.stop="discard">
								<input v-model="goodsData.number" type="digit" @blur="wetherNum()" />
							</view>
							<view class="add" @tap.stop="add">
								<fa-icon type="plus" size="20" color="#999" style="line-height: 60upx;" />
							</view>
						</view>
					</view>
				</view>
				<view class="btn">
					<view class="button" @tap="createOrder">完成</view>
				</view>
			</view>
		</view>
		...
		<view class="info-box goods-info">
			<view class="price">￥{{goodsData.retailPrice}}</view>
			<view class="title">
				{{goodsData.goodsName}}
			</view>
		</view>
		...
		<view class="info-box spec">
			<view class="row">
				...
				<view class="content">
					<view class="serviceitem">{{goodsData.product_info?goodsData.product_info:'未填写'}}</view>
				</view>
			</view>
			<!--<view class="row" @tap="showSpec(false)">
				<view class="text">规格</view>
				<view class="content">
					<view class="sp" v-if="goodsData.models">
						<view v-for="(item,index) in goodsData.spec" :key="index" :class="[index==selectSpec?'on':'']">{{item}}</view>
					</view>
					<view class="sp" v-else>未设置</view>
				</view>
		...
		<view class="description">
			...
			<view class="content">
				<rich-text :nodes="goodsData.descrip"></rich-text>
			</view>
		</view>
	</view>
</template>
...
<script>
	...
	export default {
		...
		methods: {
			...
			//增加数量
			add() {
				if (this.goodsData.number >= this.goodsData.reserve) {
					this.goodsData.number = this.goodsData.reserve;
					uni.showToast({
						title: "已超出最大库存数量",
						icon: "none"
					})
			...
			calcAnchor() {
				...
				commentsView.boundingClientRect((data) => {
					...
					this.anchorlist[1].top = 0 - headerHeight - statusbarHeight;
					this.anchorlist[2].top = 0 - headerHeight - statusbarHeight;
