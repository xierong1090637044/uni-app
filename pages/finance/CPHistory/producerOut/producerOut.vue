		...
		<scroll-view class="content" scroll-y="true" v-if="people && people.length > 0">
			<view class="info_item">
				<view v-for="(item,index) in people" :key="index" class="item">
					<navigator class="display_flex_bet" hover-class="none" v-if="item.producer":url="'/pages/finance/CPHistory/producerOutDet/producerOutDet?producerId='+item.producer.objectId">
						<view>
							<view style="font-size: 30rpx;color: #3d3d3d;font-weight: bold;" v-if="item.producer.producer_name">{{item.producer.producer_name}}</view>
							<view>
								<text>联系电话：</text>
								<text v-if="item.producer &&item.producer.producer_phone" style="font-size: 30rpx;color: #3d3d3d;font-weight: bold;">{{item.producer.producer_phone}}</text>
			...
			input_confirm(e) {
				...
				if(keyWord){
					for (let i = 0; i < that.people.length; i++) {
						if (that.people[i].producer.producer_name.split(keyWord).length > 1) {
