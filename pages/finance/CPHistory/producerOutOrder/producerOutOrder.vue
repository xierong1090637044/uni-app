	...
	<view>
		<view class='page'>
			...
			<view class="backgroundfff" style="padding: 0 20rpx;margin-top: 20rpx;">
				...
				<view>
					<navigator class="display_flex" hover-class="none" url="/pages/manage/producer/producer?type=producerFinance" style="padding: 10rpx 0;border-bottom: 1rpx solid#F7F7F7;">
			...
			<view class="backgroundfff" style="padding: 0 20rpx;margin-top: 20rpx;">
				<view class="newOrderTitle borderBot">要付款的单据</view>
				<view>
					<view class="borderBot" style="padding: 20rpx 0;width: 100%;" v-for="(item,index) in producerOutOrders" :key="index">
						...
						<view class="display_flex_bet">
							<view class="color333 display_flex">本次付款(可编辑)<text style="color: #f30;font-size: 20rpx;">*</text></view>
							<view class="display_flex">
								<input v-model="item.shouldGetMoney" placeholder="请输入本次付款" style="text-align: right;color: #f30;font-weight: bold;font-size: 28rpx;"
			...
			<view class="backgroundfff" style="padding: 0 20rpx;margin-top: 20rpx;">
				<view class="newOrderTitle borderBot">付款信息</view>
				<view>
					...
					<view class="display_flex_bet" style="padding: 10rpx 0;border-bottom: 1rpx solid#F7F7F7;">
						<view>付款人<text style="font-size: 20rpx;color: #f30;">*</text></view>
...
<script>
	...
	export default {
		...
		onLoad(options) {
			...
			that.producerOutOrders = uni.getStorageSync("producerOutOrders")
			for (let item of that.producerOutOrders) {
		...
		methods: {
			...
			inputShouldGetMoney(e, index) {
				...
				if (inputMoney > that.producerOutOrders[index].debt) {
					that.producerOutOrders[index].shouldGetMoney = that.producerOutOrders[index].debt;
				} else {
					that.producerOutOrders[index].shouldGetMoney = e.detail.value;
				}
				...
				let p1 = new Promise((resolve, reject) => {
					...
					query.get(that.account.objectId).then(res => {
						...
						res.save().then(res => {
							...
							query.set('producerOutOrders', that.producerOutOrders)
							...
							query.save().then(res => {
								...
								common.log(uni.getStorageSync("user").nickName + "操作'" + that.producer.producer_name + "'供应商付款了￥" + that.real_money +
									"元", 6, res.objectId);
									resolve("success")
							}).catch(err => {
								console.log(err)
