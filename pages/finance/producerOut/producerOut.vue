	...
	<view>
		...
		<uni-nav-bar :fixed="false" color="#333333" background-color="#FFFFFF" right-text="添加"  @click-right="goto_add" >
			<view class="input-view">
				...
				<input confirm-type="search" class="input" type="text" placeholder="输入搜索的供货商名字" @confirm="input_confirm" />
			</view>
		</uni-nav-bar>
		...
		<scroll-view class="content" scroll-y="true"  v-if="people && people.length > 0">
			<view class="info_item">
				<view v-for="(item,index) in people" :key="index" class="item">
					<navigator class="display_flex_bet" hover-class="none" :url="'/pages/manage/custom/producer_detail/producer_detail?id='+item.objectId">
						<view>
							<view style="font-size: 30rpx;color: #3d3d3d;font-weight: bold;">{{item.producer_name}}</view>
							<view>应付：<text style="color: #f30;">￥{{item.debt}}</text></view>
	...
	export default {
		...
		methods: {
			...
			select_this(type,producer) {
				...
				}else if(that.type == "producerFinance"){
					if(producer.debt == 0 || producer.debt == ''){
						uni.showToast({
							icon:"none",
							title:"没有欠款"
			...
			goto_add(){
				uni.navigateTo({
					url: "/pages/manage/custom/add/add?type=producers"
				})
			},
