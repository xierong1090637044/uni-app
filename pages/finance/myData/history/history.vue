	...
	<view>
		<view>
			...
			<scroll-view style="height: calc(100vh - 182rpx);" scroll-y>
				<view v-for="(item,index) in debt_list" :key="index" class="list_item" @click="gotoDet(item)">
					<block>
	...
	export default {
		...
		methods: {
			...
			gotoDet(item){
				...
				if(item.type == -1){
					uni.navigateTo({
						url:"/pages/finance/CPHistory/customInHistoryDet/customInHistoryDet?id="+item.objectId
					})
				}else if(item.type == 1){
					uni.navigateTo({
						url:"/pages/finance/CPHistory/producerOutHistoryDet/producerOutHistoryDet?id="+item.objectId
					})
				}
			},
