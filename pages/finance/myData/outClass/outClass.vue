	...
	<view class='page'>
		<view class="content">
			...
			<view class="right_content">
				<view style="height: calc(100vh - 80rpx);overflow: scroll;">
					<view v-for="(item,index) in financeSecondClass" :key="index" class="class_item" style="color: #333333;">
		...
		methods: {
			...
			get_second_category(id) {
				...
				query.relation('financeSecondClass').then(res => {
					console.log(res);
					that.loading = false;
					that.financeSecondClass = res.results;
				})
			},
			...
			showoption(id, type, value) {
				...
				uni.showActionSheet({
					...
					success: function(res) {
						...
						if (res.tapIndex == 0) {
							if (type === 1) {
								that.delete(id, "financeFristClass")
							} else {
								that.delete(id, "financeSecondClass")
							}
			...
			edit() {
				...
				if (that.Popup_title === "一级分类") {
					dbname = "financeFristClass";
				} else {
					dbname = "financeSecondClass";
				}
