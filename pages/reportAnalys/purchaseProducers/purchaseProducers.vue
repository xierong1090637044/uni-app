<template>
	<view>
		...
		<view>
			...
			<view v-for="(item,index) in PurchaseGoodList" :key="index" class="listItem" v-if="item.producer && item.producer.producer_name">
				<view style="border-bottom: 1rpx solid#ddd;padding-bottom: 20rpx;">{{item.producer.producer_name}}</view>
				<view v-if="current == 0" style="padding-top: 20rpx;" class="display_flex_bet">
					<view>采购数量：{{item._sumNum}}</view>
					<view>采购金额：{{item._sumTotal_money}}</view>
				</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			input_confirm(e) {
				that.params.producerName = e.detail.value
				that.getPurchaseGoodList()
			},

			getPurchaseGoodList() {
				that.$http.Post("reportAnalys_purchaseProducer", that.params).then(res => {
