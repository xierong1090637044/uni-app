	...
	<view class="page">
		...
		<view v-else>
			...
			<view class="second">
				...
				<view class="second_one">
					<view>材质: <text class="second_right_text">{{product.packingModel?product.packingModel:"未填写"}}</text></view>
					<view>规格: <text class="second_right_text">{{product.packageContent?product.packageContent:"未填写"}}</text></view>
					<view>单位: <text class="second_right_text">{{product.packingUnit?product.packingUnit:"未填写"}}</text></view>
					...
					<view v-if="product.fristClass && product.fristClass.class_text">所属一级分类 <text style="margin-left: 20rpx;color: #3D3D3D;">{{product.fristClass.class_text}}</text></view>
					<view v-if="product.secondClass && product.secondClass.class_text">所属二级分类 <text style="margin-left: 20rpx;color: #3D3D3D;">{{product.secondClass.class_text}}</text></view>
				</view>
				...
				<view v-if="get_reserve_checked" class="second_one" v-for="(item,index) in product.stocks" :key="index">
					...
					<navigator class="display_flex_bet" hover-class="none" :url="'/pages/manage/good_det/bad_history/bad_history?id='+item.good_id+'&type=matter'">
					...
					<view class="display_flex">
						...
						<navigator hover-class="none" :url="'../operations/operations?objectId='+item.good_id+'&goodsName='+product.goodsName" class="opion_item">此物料的操作记录</navigator>-->
					</view>
					<view class="display_flex">
			...
			<view class="qrimg" v-if="is_show">
				...
				<view style="margin-top: 20%;" @tap="saveQrcode">
					...
					<view style="color: #fff;margin-top: 30rpx;font-size: 32rpx;">物料:{{product.goodsName}}</view>
			...
			<uni-popup :show="bad_numshow" type="top" mode="fixed" @hidePopup="bad_numshow = false" class="popup">
				<view class="popup_content">
					...
					<view style="margin-bottom: 20rpx;"><input placeholder="请输入该物料的货损数量" class="popup_input" v-model="badnum.num" /></view>
